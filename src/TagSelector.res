...
let make = (
  ...
  ~forceOpen: option<bool>=?,
  ~showSelectedChips: option<bool>=?,
  ~showInput: option<bool>=?,
) => {
  let forceOpen_ = switch forceOpen {
  | Some(v) => v
  | None => false
  }
  let showSelectedChips_ = switch showSelectedChips {
  | Some(v) => v
  | None => true
  }
  let showInput_ = switch showInput {
  | Some(v) => v
  | None => true
  }
  ...
  let handleKeyDown = e => {
    let colCount: int = %raw(
      "(typeof window !== 'undefined' ? (window.innerWidth >= 1280 ? 3 : (window.innerWidth >= 640 ? 2 : 1)) : 1)"
    )
    ...
    {forceOpen_ || isOpen
      ? <>
          ...
          {isAllSelected
            ...
            ? <div
              className="mt-1 w-full bg-white rounded-md border border-gray-200 max-h-72 overflow-y-auto p-2"
              onKeyDown={handleKeyDown}
            >
            ...
            : <div
                className="mt-1 w-full rounded-md border border-gray-200 px-3 py-2 text-sm text-gray-400"
              >
