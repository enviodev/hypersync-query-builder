...
test("generateEnglishDescription - combined address and topics", () => {
  let state = {addresses: ["0x1234"], topics: [["0xabcd"]]}
  let result = generateEnglishDescription(state)
...
test("generateBooleanHierarchy - multiple addresses", () => {
  let state = {addresses: ["0x1234", "0x5678"], topics: []}
  let result = generateBooleanHierarchy(state)
...
test("generateBooleanHierarchy - address and single topic", () => {
  let state = {addresses: ["0x1234567890"], topics: [["0xabcd"]]}
  let result = generateBooleanHierarchy(state)
...
test("generateBooleanHierarchy - user's multiple addresses with multiple topic arrays", () => {
  let state = {
    addresses: ["0xa", "0xb"],
    topics: [["0x1"], [], ["0x2", "0x3"]]
  }
